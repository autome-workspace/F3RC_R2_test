...
bool BNO055::loadCalibration() {
    ...
    // EEPROMから1バイトずつ読み込み
    for (int i = 0; i < BNO055_CALIBRATION_DATA_SIZE; i++) {
        calibrationData[i] = EEPROM.read(EEPROM_OFFSET + i);
        // データが存在することを示すフラグ
        if (calibrationData[i] != 0xFF) {
            dataFound = true;
        }
    }
    ...
    if (dataFound) {
        // 読み込んだデータをBNO055に適用
        // setSensorOffsets() のように直接レジスタに書き込む処理を実装
        // bno055_set_sensor_offsets() のような内部関数を呼び出す
        return true;
    }
    return false;
}
...
bool BNO055::saveCalibration() {
    ...
    // 全てのセンサーが完全にキャリブレーションされているか確認
    if (system == 3 && gyro == 3 && accel == 3 && mag == 3) {
        // BNO055からキャリブレーションデータを取得
        uint8_t calibrationData[BNO055_CALIBRATION_DATA_SIZE];
        // getSensorOffsets()のように、直接レジスタから読み取る処理を実装
        // bno055_get_sensor_offsets() のような内部関数を呼び出す

        // EEPROMに1バイトずつ書き込み
        for (int i = 0; i < BNO055_CALIBRATION_DATA_SIZE; i++) {
            EEPROM.write(EEPROM_OFFSET + i, calibrationData[i]);
        }
        EEPROM.commit(); // ESP32では必須
        return true;
    }
    return false;
}