...
bool Drive::drive(float targetX, float targetY, float targetRadians) {
    ...
    // 制限を超える場合
    if (SPD_LIMIT > _ROBOT_SPD_LIMIT) SPD_LIMIT = _ROBOT_SPD_LIMIT;
    if (ACC_LIMIT > _ROBOT_ACC_LIMIT) ACC_LIMIT = _ROBOT_ACC_LIMIT;

    float cv_squared = cvx*cvx + cvy*cvy;
